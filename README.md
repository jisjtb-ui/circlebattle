...
動画を作るための **1 対 1 のモード**が別にあります (`video.html`)。LIVE 版とは
コードも設定も完全に分かれていて、こちらを触っても配信用の画面は変わりません
(→ [14. VIDEO BATTLE MODE](#14-video-battle-mode動画用))。
...
| **入室** | **レベル 20 の円をもらえる**（1 人 1 回だけ）。大砲から撃ち出され、金色の輪で光ります |
...
小さく映るので、**顔とレベルが読めること**を、画面に何個入るかより優先して
半径を 1.5 倍（20 → 30、Lv100 の上限は 48 → 72）にしました。
...
配信に映すのは `game.html` です。**画面いっぱいがスタジアム**で、
そのうえに表示を重ねます。基準は TikTok LIVE の縦画面 (1080×1920 = 9:16)。

```
┌──────────────────────────┐  ← ネオンの外枠 (色が流れます)
│ WAVE 3          ● LIVE   │
│ ENEMIES DEFEATED  1,234  │
│ ENEMIES 12 CIRCLES 27    │
│ BOSS ▓▓▓▓▓▓░░░░  5,168   │  ← 大物が居る間だけ
│ ┌────────────┐           │
│ │ TOP 5      │   ○   ●   │  ← 左に重ねます。盤面が透けます
│ │ 1 @A   128 │       ○   │
│ │ …          │  ●        │
│ │ @Z #23 …   │           │  ← 圏外の人の順位
│ └────────────┘      ○    │
...
## 14. VIDEO BATTLE MODE（動画用）

`video.html` を開くと始まる、**RED 対 BLUE の 1 対 1**です。視聴者イベントも
ランキングも使いません。動画に撮って出すことだけを考えた画面です。
...
```
   ...
   BLUE ████████░░░░░░░░░░░░
        ...
        ┌────────────────┐
        │   RED  ⚔       │   9:16
        │        🔫 BLUE │   HP バーも WINNER も
        │                │   すべて canvas の中
        └────────────────┘
```

**LIVE 版のファイルは 1 つも読み込みません。** `js/config.js` も `js/game.js` も
`js/audio.js` も触っていないので、このモードをどういじっても
`index.html` / `game.html` の動きは変わりません。

| 動画モード | LIVE 版の対応するもの |
| --- | --- |
| `js/video-config.js` | `js/config.js` |
| `js/video-battle.js` | `js/game.js` |
| `js/video-renderer.js` | `js/renderer.js` |
| `js/video-audio.js` + `js/video-sound-bank.js` | `js/audio.js` |
| `js/video-sound-director.js` | (LIVE 版では `main.js` の中) |
| `js/video-recorder.js` | (LIVE 版には無い) |
| `video.html` / `css/video.css` | `game.html` / `css/game.css` |

### 14.1 HP バー

画面の上に RED と BLUE の HP が常に出ます。**数値は出しません。**
長さだけで優劣が分かることが目的なので、数字があるとかえって読む時間がかかります。

- RED は左から、BLUE は右から伸びます (対戦しているように見せるため)。
- 減り方は**すぐには追いつきません**。短いアニメーションで滑らかに減ります。
- 減ったぶんは**白い帯**としてしばらく残ります。これがあるので、
  大ダメージのときは「今どれだけ減ったか」がひと目で分かります。
- 10% ごとの目盛りを薄く入れてあります。数字を出さずに割合を読ませるためです。

危険域では出方が変わります。画面全体を赤く染めるようなことはしません
(盤面が見えなくなると、何が起きているのか分からなくなるためです)。

| HP | 見え方 |
| --- | --- |
| 30% 以下 | バーが強く光る |
| 15% 以下 | バーの外に枠が出て脈打つ。その円のまわりにもリング |
| 5% 以下 | 白く速く明滅し、`DANGER` が出る。円のリングも速くなる |

### 14.2 攻撃 → 命中 → HP という順番

一番大事にしているのは**イベントの順番**です。「攻撃したから音が鳴る」ではありません。

```
攻撃を始めた   → 'shot'    射出音
当たらなかった → 'miss'    音は鳴りません (外れたことは音の不在で伝わります)
実際に当たった → 'hit'     ヒット音 + HP 減少 + 演出
```

HP が減るのは `'hit'` のときだけです。だから視聴者は、音を聞くだけで
「当たったのか / 避けたのか」が分かります。振りかぶっている間に相手が離れれば
外れるので、「ギリギリ外れる」も自然に起きます。

当たった瞬間に出るのは、円の一瞬のフラッシュ・短いヒットストップ (55ms)・
小さな画面振動・ダメージ方向へのノックバックです。大ダメージ (最大 HP の 9% 以上)
では、止める時間も振動もフラッシュも強くなります。どれも短くしてあります
(長い演出はテンポを殺すためです)。

### 14.3 武器

6 種類です。落ちている武器を拾うと持ち替わります。射出音とヒット音は別々に持ちます。

| 武器 | 射出 | ヒット | 特徴 |
| --- | --- | --- | --- |
| HAMMER | 低く重い「ドン」+ 重量感のあるスイング | 「ドゴン」 | 1 発が重い。当たれば必ず大ダメージ |
| LONG SWORD | 鋭い「シュッ」 | 金属質の斬撃音 | 手数が多い |
| INJECTION NEEDLE | ごく短い「チッ」(一番静か) | 小さく鋭い音 → 毒付与音 | 刺さったあとも削り続ける |
| PISTOL | 単発の「パンッ」 | 明確なインパクト | 連射が遅く 1 発ずつ聞き取れる |
| SHOTGUN | 強い低音 + 散っていく粒 | 3 連の衝撃 | 6 粒。近いほど強い |
| LASER | 「ウィーン」という充填音 | 継続ダメージ音 | 照射中はループ音が鳴り続ける |

落ちる武器は**全種類を 1 巡させてから混ぜ直します**。毎回でたらめに選ぶと、
1 本の動画で HAMMER が一度も出ない回ができてしまい、一番重い一撃が最後まで来ません。
...
- SHOTGUN の粒も LASER の照射も、そのまま鳴らすと 1 秒に何十回も重なります。
  **音ごとの最短間隔**と**1 秒あたりの上限**で間引きます。LASER のダメージ音は
  毎フレームではなく 190ms ごとです。
- 終盤ほど音が厚くなります (`audio.intensity`)。序盤は軽く、終盤は迫力が出ます。
  ダメージそのものも時間とともに伸びるので、後半ほど 1 発が重くなります。
- **最後の一撃の前に、一瞬だけ音が引きます。** HP が 5% を割った瞬間に
  張りつめた音が鳴り、そのあと 0.42 秒だけ効果音が下がります。次に来る一撃が
  実際の音量以上に大きく聞こえます。
- 決着では撃破音・敗北音・勝利音が鳴ります。**WINNER の表示は `ko` と同じ時刻から
  動く**ので、音と表示がずれません。
- BGM を入れると、大きい音が鳴った瞬間だけ BGM が下がります (サイドチェイン)。
  効果音が埋もれません。

### 14.5 音源の差し替え

音は既定では**その場で合成**します (ファイルが 1 つも無くても鳴ります)。
[`sounds/`](sounds/) に同じ名前のファイルを置くと、**その音だけ**が差し替わります。

```
sounds/hammer-shot.wav   sounds/hammer-hit.wav
sounds/sword-shot.wav    sounds/sword-hit.wav
sounds/needle-shot.wav   sounds/needle-hit.wav    sounds/needle-poison.wav
sounds/pistol-shot.wav   sounds/pistol-hit.wav
sounds/shotgun-shot.wav  sounds/shotgun-hit.wav
sounds/laser-start.wav   sounds/laser-loop.wav    sounds/laser-hit.wav
sounds/laser-end.wav     sounds/item-pickup.wav   sounds/bounce.wav
sounds/danger.wav        sounds/final-charge.wav  sounds/ko-impact.wav
sounds/defeat.wav        sounds/victory.wav
```

読めなかったファイルは黙って合成に落ちるので、音が消えることはありません。
パネルの `音源:` の行に「ファイル ○ / 合成 ○」と出ます。
詳しくは [`sounds/README.md`](sounds/README.md)。

### 14.6 録画

**[ REC ]** で、画面と**ゲーム音**をまとめて webm に書き出します。効果音・ヒット音・
勝利音まで入った、そのまま出せる状態の動画になります。

撮っているのは canvas だけです。だから HP バーも WINNER もすべて canvas に描いてあり、
操作パネル (音量・BGM・録画ボタン) は**動画に写りません**。`H` キーか
**[ HIDE UI ]** で画面からも消せます。

- `MASTER` / `SFX` / `MUSIC` の 3 つで音量を決めます。設定は次に開いたときも残ります。
- BGM を読み込んでおくと、同じミキサーを通って録画にも一緒に入ります。
- `AUTO REMATCH` を入れておくと、決着の 6 秒後に次の試合が始まります。
  長い素材を撮りっぱなしにできます。
- 出力は webm (VP9 + Opus。対応していないブラウザでは VP8 に落ちます)。

### 14.7 コンソールから触る

```js
CB.video.battle.fighters.red.hp = 50;      // 決着まわりの見え方を確かめる
CB.video.battle.equip(CB.video.battle.fighters.blue, 'hammer');
CB.video.battle.spawnItem('laser');        // 武器を落とす
CB.video.battle.reset();                   // 仕切り直す
CB.video.audio.setSfxVolume(0.5);
```

調整値はすべて [`js/video-config.js`](js/video-config.js) にあります。
HP も武器の強さも演出の長さも音量も、コードに数値はありません。
