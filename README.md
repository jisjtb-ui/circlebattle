...
- 円は**足し算**です。10 個出ている人が 100 LIKE すれば 20 個になります。
  入れ替わるのは、その人が上限 (`viewers.limits.maxPerUser`、既定 40 個) に達したときだけです。
...
- 向きを変えるのは反射のときだけ。追いかけたり、ふらついたりしません
- 重なったときだけ位置を押し離します（速度には触らないので、向きは変わりません）

盤面が読めるので「あの円があの敵に当たりそう」が見て分かります。
追いかける動きに戻したいときは `js/config.js` で:
