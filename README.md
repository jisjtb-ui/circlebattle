...
**押したぶん・贈ったぶんは、いくらでも数字になります。**
「これ以上やっても同じ」が見えた瞬間、その人の手は止まるからです。

| 力 | 1 ポイント | どこまで |
| --- | --- | --- |
| HP | +70 | 止まりません |
| 攻撃力 | +6 | 止まりません |
| ドレイン | – | 100% に近づくだけ（超えません） |
| スパイク | 棘 1 本 | 止まりません |

ドレインだけ別扱いなのは**割合**だからです。足し算で 100% を超えると
「殴るほど回復が増えて減らない」円ができて、誰にも倒せなくなります。

大きさも止まりませんが、伸び方は緩やかです。
...
| 1,000 | 74 |
| 10,000（10 万いいね） | 99 |
| 1,000,000 | 180（画面の 1/3） |

**ギフトの一人勝ちは強まります。** 100 人 10 分の実測で、ギフト中心の人の
平均スコアは他の 4 倍から **9 倍**になり、盤面に残る円は 68 → 54 に減りました
（強い人が他を倒すため）。抑えたいときは操作画面の **PVP DAMAGE** を下げて
ください。敵を狩る強さはそのままで、視聴者どうしの削り合いだけ穏やかになります。
...
- アイコンが見える大きさになるよう、視聴者円は直径 60px 以上（1080 幅換算）にしてあります。
  貯めた力が多いほど円も大きくなります（`viewers.base.radius` と `viewers.size`）

### 残り HP は円の真ん中

**円の真ん中に、今の HP を大きく出します。**
...
```
   ...
   │   128   │  ← 貯めた力の合計
   └─────────┘
     ふちの太いリングも同じ色で残量を出します
```

視聴者どうしも戦うようになったので、いちばん見たい数字は HP です。
外周のリングだけだと、密集したときに隣の円のリングと混ざって読めません。

- **色だけで危ない円が分かります**（残り 50% で黄、25% で赤）
- リングには黒い下地を敷いてあります。無いと、残りが少ないときに
  「どこまであったのか」が分からず、あと少しなのかが読めません
- **プロフィール画像の上に重なります。** 顔を隠さないことより、
  あとどれだけ耐えられるかが読めることを優先しました。下に敷く帯は
  横長なので、顔の上下は透けたまま残ります
...
- **拾って弱くなることはありません。** エンジンは「今の値より大きいほう」しか採らないので、
  ポイントも HP も攻撃力も大きさも速さも下がりません。とても強い円が POWER を
...
stats: {
  hp:     { per: 70 },     // 1 ポイントで HP +70 (頭打ちなし)
  attack: { per: 6  },
  drain:  { per: 0.05 },   // 1 - 1 / (1 + ポイント × per)。100% に近づくだけ
  spike:  { per: 1  }
}
```
