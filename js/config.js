...
(function (global) {
  ...
  var CONFIG = {
     ...
     * 進行役 (Director)。
     *
     * **ラウンドの残り時間**を数え、ときどき特殊イベントを起こします。
     * 時間が来たら結果を出して全部リセットし、次のラウンドが始まります。
     *
     * 以前の `WAVE 1 → 2 → 3 …` はやめました。数が積み上がるほど、
     * あとから来た人には「もう 7 まで進んでいる」に見えて入りづらくなります。
     * 決まった長さで区切れば、**どこから見ても次は最初から**です。
     */
    director: {
      enabled: true,

      /**
       * 1 ラウンドの長さ (ミリ秒)。ここが来たら**全部リセット**します。
       *
       * 円もランキングも消えるので、短すぎると育てる余地がなく、
       * 長すぎると途中から来た人が追いつけません。10 分が既定です。
       * 0 にするとリセットしません (終わりのない 1 本勝負)。
       * 操作画面の ROUND TIME で変えられます。
       */
      roundMs: 600000,
      /** 結果を出しておく時間 (ミリ秒)。この間もゲームは動いています。 */
      resultMs: 6000,

      /** 特殊イベントを起こすか判定する間隔 (ミリ秒)。 */
      eventEveryMs: 90000,
      /**
       * 判定のたびに特殊イベントが起きる確率。
