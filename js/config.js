...
(function (global) {
  ...
  var CONFIG = {
    ...
    viewers: {
       ...
       * ポイント → 実際の数値。
       *
       * **頭打ちはありません。** 押した / 贈ったぶんは必ず数字になります。
       * 「これ以上やっても同じ」が見えた瞬間、その人の行動は止まります。
       *
       * 上限を外しても壊れないのは、**どの道も同じように伸びる**ためです。
       * いいねを積んだ人は HP と一緒に棘の威力も伸びる (棘は最大 HP から
       * 出しています) ので、攻撃力だけが青天井になるわけではありません。
       *
       * ただし**割合** (ドレイン) だけは 1 を超えると意味が壊れるので、
       * 伸ばし続けても 1 に近づくだけ、という形にしてあります (下記)。
       */
      stats: {
        /** いいね。1 ポイントで最大 HP がこれだけ増えます。 */
        hp:     { per: 70,   label: 'HP' },
        /** ギフト。1 ポイントで攻撃力がこれだけ増えます。 */
        attack: { per: 6,    label: 'ATK' },
        /**
         * シェア。与ダメージを回復に回す割合。
         *
         *   drain = 1 - 1 / (1 + ポイント × per)
         *
         * 足すのではなく 1 へ近づけます。1 を超えると「殴るほど回復が
         * 増えて減らない」になり、誰にも倒せなくなるためです。
         * 近づき方は緩やかなので、何回シェアしても必ず少しは効きます。
         */
        drain:  { per: 0.05, label: 'DRAIN' },
        /** フォロー。棘の本数のもとになります (長さと威力は HP で伸びます)。 */
        spike:  { per: 1,    label: 'SPIKE' }
      },
       ...
       *
       *   radius = base.radius * (1 + 合計ポイント) ^ exp
       *
       * どの道で強くなった人も、強いほど大きく見えます。
       * **こちらも頭打ちはありません。** 指数が小さいので伸び方は緩やかで、
       * 上限を置かなくても画面を埋め尽くすことはありません:
       *
       *   150 ポイント (150 コイン)      半径 58
       *   1,000 ポイント                 半径 74
       *   10,000 ポイント (10 万いいね)  半径 99
       *   1,000,000 ポイント             半径 180  (1080 幅の 1/3)
       */
      size: {
        exp: 0.13
      },
